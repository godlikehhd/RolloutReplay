...
def dataprotoitem_to_dataproto(item) -> DataProto:
...
def select_buffer(buffer, config):
    ...
    n = len(buffer) // 8
    print(f"After removing all 0 or all 1, buffer size: {n}")
    ...
    selected_data = dataprotoitem_to_dataproto(buffer[selected_mask]) # (1-sigma)*B

    remaining_budget = config.data.buffer_size - int(config.data.sigma * config.data.train_batch_size) # 2048 - 256  
...
def deduplicate_buffer(buffer):
    ...
    mask = mask.tolist()
    deduped_buffer = dataprotoitem_to_dataproto(buffer[mask])
    print(f"After deduplication, buffer size: {len(deduped_buffer) // 8}")
    return deduped_buffer

class ValueTracker:
    def __init__(self, dataset, half_life_steps: int = 10_000):
        """
        :param half_life_steps: 半衰期步数：当 t = half_life_steps 时，rho ≈ 0.5
        """
        self.half_life = half_life_steps
        # 存储每个 prompt x 的 alpha, beta, last_update_step
        # key 可以是 prompt_id 或 prompt_hash
        self._storage = defaultdict(lambda: {"alpha": 1.0, "beta": 1.0, "last_step": 0})
        self._global_step = 0
    ...
    def _init(self):
        for i in range(len(self.dataset)):
            prompt_id = self.dataset[i]["extra_info"]["index"]
            entry = self._storage[prompt_id]
            

    def _rho(self, last_step: int) -> float:
        """
        计算折扣因子 rho(x) = 2^{-(current_step - last_step) / half_life}
        """
        delta = self._global_step - last_step
        return 2 ** (- delta / self.half_life)

    def update(self, batch: DataProto):
        """
        在第 self._global_step 步上，prompt_id 得到 reward (0.0 或 1.0)
        """
        prompt_ids = batch.non_tensor_batch["extra_info"]
        rewards = batch.batch["token_level_rewards"]
        rewards = rewards.sum(dim=-1)
        for i in range(len(prompt_ids)):
            prompt_id = prompt_ids[i]["index"]
            reward = rewards[i]
            entry = self._storage[prompt_id]
            rho = self._rho(entry["last_step"])
            entry["alpha"] = rho * entry["alpha"] + reward
            entry["beta"] = rho * entry["beta"] + (1.0 - reward)
            entry["last_step"] = self._global_step + i

    def value(self, batch: DataProto):
        """
        返回当前 prompt_id 的估计成功概率 \hat v(x) = alpha / (alpha + beta)
        若 prompt_id 从未出现，则 = 0.5 (alpha=1,beta=1)
        """
        prompt_ids = batch.non_tensor_batch["extra_info"]
        values = []
        for prompt_id in prompt_ids:
            entry = self._storage[prompt_id["index"]]
            values.append(entry["alpha"] / (entry["alpha"] + entry["beta"]))
        return torch.tensor(values, dtype=torch.float32)
    
    def weights(self):
        weights = []
        for prompt_id in self._storage:
            entry = self._storage[prompt_id]
            value = entry["alpha"] / (entry["alpha"] + entry["beta"])
            weight = 1 - (value - 0.5) ** 2
...
def compute_advantage(
    ...
    if adv_estimator == AdvantageEstimator.GAE:
        ...
        spo_calculation_mask = data.batch["response_mask"]
        advantages, returns = core_algos.compute_spo_outcome_advantage(
            token_level_rewards=data.batch["token_level_rewards"],
            response_mask=spo_calculation_mask,
            values = data.batch["values"],
            index=data.non_tensor_batch["uid"],
            config=config,
        )
...
class RayPPOTrainer:
    ...
    def __init__(
        ...
        self.value_tracker = ValueTracker(train_dataset)
    ...
    def _update_sampler(self):
        self.train_sampler.weights = self.value_tracker.weights()
    ...
    def _create_dataloader(self, train_dataset, val_dataset, collate_fn, train_sampler: Optional[Sampler]):
        ...
        self.train_sampler = create_rl_sampler(self.config.data, self.train_dataset, self.value_tracker)
        print("create train_sampler")
    ...
    def _dump_generations(self, inputs, outputs, gts, scores, idxs, reward_extra_infos_dict, dump_path):
    ...
    def _validate(self):
        print(f"Validating at global step {self.global_steps}")
        data_source_lst = []
        ...
        if self.config.trainer.val_only:
            ...
            filename = os.path.join(val_data_dir, f"0.jsonl")
            if os.path.exists(filename):
                with open(filename, "r") as f:
                    lines = len(f.readlines())
                current_step = (lines - 240) // 128
            else:
                current_step = 0
        else:
            current_step = 0
        for test_data in tqdm(self.val_dataloader, desc="Validation"):
            ...
            extra_infos = [item.non_tensor_batch.get("extra_info", None) for item in test_batch]
            idxs = [item.get("index", 0) for item in extra_infos]
            ...
            #         )
            #         del sample_inputs, sample_outputs, sample_gts, sample_scores, sample_idxs, reward_extra_infos_dict
            #         sample_inputs = []
            #         sample_outputs = []
            #         sample_gts = []
            #         sample_scores = []
    ...
    def reward_shaping_rr(self, batch: DataProto) -> DataProto:
        ...
        indices = np.array(batch.non_tensor_batch['uid'])
    ...
    def select_replay_batch(self, batch: DataProto, replay_n: int) -> DataProto:
        """Select all replay buffer samples that match the given batch's indices.
        
        If one index in batch appears multiple times in replay_buffer, 
        all matching trajectories are selected.
        """
        # print("batch.non_tensor_batch.keys():", batch.non_tensor_batch.keys())
        # print("self.replay_buffer.non_tensor_batch.keys():", self.replay_buffer.non_tensor_batch.keys())

        # 取出目标和缓冲区的索引
        index_target = batch.non_tensor_batch["index"]
        index_source = self.replay_buffer.non_tensor_batch["index"]
        token_level_scores_source = batch.batch["token_level_scores"]
        token_level_scores_target = self.replay_buffer.batch["token_level_scores"]
        ...
        # 找到所有目标 index 对应的样本索引
        selected_positions = []
        for idx in index_target:
            if idx in self.replay_index_to_positions:
                selected_positions.extend(self.replay_index_to_positions[idx][:replay_n])
                self.replay_index_to_positions[idx] = self.replay_index_to_positions[idx][replay_n:] if len(self.replay_index_to_positions[idx]) > replay_n else []
            else:
        ...
        # 从 replay buffer 中选取这些样本
        selected_batch = self.replay_buffer.select_idxs(selected_positions)
    ...
    def fit(self):
        ...
        if self.config.trainer.do_replay:
            if self.global_steps == 1:
                ...
                with open(local_latest_buffer_iteration, 'r') as f:
                    latest_global_steps = int(f.read().strip())
                    assert self.global_steps == latest_global_steps + 1, "Buffer iteration mismatch"
                replay_buffer_dir = os.path.join(self.config.trainer.default_local_dir, 'replay_buffer_{}.pkl'.format(latest_global_steps))
        ...
        for epoch in range(self.config.trainer.total_epochs):
            ...
            for batch_dict in self.train_dataloader:
                ...
                values = self.value_tracker.value(batch)
                batch.batch["values"] = values
                
                is_last_step = self.global_steps >= self.total_training_steps
                if (self.config.trainer.do_replay and epoch == 0) or self.config.trainer.do_replay == False:
                    ...
                    with marked_timer("step", timing_raw):
                        ...
                        with marked_timer("adv", timing_raw, color="brown"):
                            ...
                            self.value_tracker.update(batch)
                    ...
                    print("Replaying...")
                    new_n = self.config.actor_rollout_ref.rollout.n * (1 - self.config.trainer.replay_ratio)
                    replay_n = self.config.actor_rollout_ref.rollout.n * self.config.trainer.replay_ratio
                    ...
                    gen_batch = gen_batch.repeat(repeat_times=new_n, interleave=True)
                    ...
                    with marked_timer("step", timing_raw):
                        ...
                        with marked_timer("gen", timing_raw, color="red"):
                            ...
                            gen_batch_output.meta_info.pop("timing", None)
                        # repeat to align with repeated responses in rollout
                        batch = batch.repeat(repeat_times=new_n, interleave=True)
                        batch = batch.union(gen_batch_output)
                        ...
                        with marked_timer("adv", timing_raw, color="brown"):
                            ...
                            replay_batch_adv = self.select_replay_batch(batch, replay_n)
                            batch = DataProto.concat([batch, replay_batch_adv])
                            batch.meta_info["global_token_num"] = torch.sum(batch.batch["attention_mask"], dim=-1).tolist()
            ...
            self.replay_bufffer = self.replay_buffer_new
            self.replay_buffer_new = None
