...
TEST_DATA_PATH="[$(echo ${WORKING_ROOT_DIR}/data/qwen-math/deepscaler/train_20k.parquet)]"
# TEST_DATA_PATH="[$(echo ${WORKING_ROOT_DIR}/data/eval-data-no-system-prompt/aime24.parquet)]"
...
PROJECT_NAME=baseline-verl-spo-test
EXPERIMENT_NAME=${EXP_NOTE}-${MODEL_NAME}-${DATA_NAME}-NODE${WORLD_SIZE}
OUTPUT_DIR=/local_data/shares/checkpoints/${PROJECT_NAME}/${EXPERIMENT_NAME}
mkdir -p ${OUTPUT_DIR}
...
python3   -m verl.trainer.main_ppo \
    ...
    data.weighted_random_sampler=True \
    ...
    algorithm.normalize_batch_advantage=True \
